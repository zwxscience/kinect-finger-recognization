...
void XnVPointDrawer::OnPointUpdate(const XnVHandPointContext* cxt)
{
	...
	//update最新位置
	HandPostion.X=ptProjective.X;
	HandPostion.Y=ptProjective.Y;
	HandPostion.Z=ptProjective.Z;
...
GLuint initTexture(void** buf, int& width, int& height)
{
	...
	width = getClosestPowerOfTwo(width);//chu2
...
void DrawRectangle(float topLeftX, float topLeftY, float bottomRightX, float bottomRightY)
{
	...
	glVertexPointer(2, GL_FLOAT, 0, verts);//定义丁点数组坐标
...
void DrawTexture(float topLeftX, float topLeftY, float bottomRightX, float bottomRightY)
{
	...
	glTexCoordPointer(2, GL_FLOAT, 0, texcoords);//纹理映射坐标
...
void DrawDepthMap(const xn::DepthMetaData& dm)
{
	...
	if(!bInitialized)
	{
		...
		texcoords[0] = texXpos, texcoords[1] = texYpos, texcoords[2] = texXpos, texcoords[7] = texYpos;//纹理顶点
	...
	{
		g_pDepthHist[nIndex] += g_pDepthHist[nIndex-1];//累计深度
	}
	if (nNumberOfPoints)
	{
		for (nIndex=1; nIndex<MAX_DEPTH; nIndex++)
	...
	pDepth = dm.Data();
	{
		...
		for (nY=0; nY<g_nYRes; nY++)
		{
			for (nX=0; nX < g_nXRes; nX++, nIndex++)
			{
				...
				if (nValue != 0)
				{
					nHistValue = g_pDepthHist[nValue];//替换
...
void XnVPointDrawer::Draw() const
{
		...
		++PointIterator)
	{
		...
		glColor4f(Colors[nColor][0],
				...
				1.0f);
		glPointSize(8);
		glVertexPointer(3, GL_FLOAT, 0, m_pfPositionBuffer);//存放顶点位置
	//glDrawArrays(GL_LINE_STRIP, 0, i);

		glDrawArrays(GL_POINTS, 0, 1);	
		//{//设置鼠标跟随
		//	tagPOINT cursorpost;
		//cursorpost.x=m_pfPositionBuffer[0];
		//cursorpost.y=m_pfPositionBuffer[1];
		//	SetCursorPos(cursorpost.x,cursorpost.y);
		//}
		glFlush();
	}
}
...
void PrintSessionState(SessionState eState)
{
	...
	case IN_SESSION:{
		...
		sprintf(strLabel, "Tracking hands");
		break;}
	case NOT_IN_SESSION:
